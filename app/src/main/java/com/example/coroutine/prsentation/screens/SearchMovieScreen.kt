...
fun SearchMoviesScreen(navController: NavHostController,
    ...
    var moivePagingItems = popularMoviesState.collectAsLazyPagingItems()
                ...
                LazyVerticalGrid(columns = GridCells.Fixed(3),
                    ...
                    items(moviePagingItems.itemCount){ index ->
                        if(moviePagingItems[index]?.adult==false){
                            AsyncImage(model = "${MOVIE_IMAGE_BASE_URL}${imageSize.original}/${moviePagingItems[index]?.posterPath}"
                                , contentDescription = "",
...
/* var searchText by remember { mutableStateOf("") }
 ...
 Column {
   ...
   //  ShowMoviesSearch.iovoke(searchText)
 }

// ShowMoviesSearch( searchText  , navController)
...
/*
@Composable
fun ShowMoviesSearch(MoiveName : String, navController: NavHostController) val SEarchMovieState = hiltViewModel<SearchMovieViewModel>()
 LaunchedEffect(MoiveName) {

     SEarchMovieState.getMovieSearch(MoiveName)
 }

 val moviePagingItems =  SEarchMovieState.searchMovieState.collectAsState()
 when (val data = moviePagingItems.value){
